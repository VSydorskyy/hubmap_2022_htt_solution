...
class SWACallback(Callback):
    ...
    def _save_checkpoint(self, runner):
        ...
        save_dict = {
            i: (s, m, e) for i, (s, m, e) in enumerate(zip(self.scores, self.model_checkpoints, self.best_epochs))
        }
    ...
    def on_epoch_end(self, runner):
        if self.loader_key is None:
            ...
            epoch_metric = runner.epoch_metrics[self.loader_key][self.logging_metric]
        ...
        if self.maximize:
            if np.min(self.scores) < epoch_metric:
                ...
                self.model_checkpoints[min_value_index] = self._put_state_dict_on_cpu(
                    (runner.model if self.model_key is None else runner.model[self.model_key]).state_dict()
                )
            ...
            if np.max(self.scores) > epoch_metric:
                ...
                self.model_checkpoints[max_value_index] = self._put_state_dict_on_cpu(
                    (runner.model if self.model_key is None else runner.model[self.model_key]).state_dict()
                )
        ...
        if self.verbose:
            if self.loader_key is None:
                msg = "Best models scores by {} : {}".format(self.logging_metric, self.scores)
            else:
                msg = "Best models scores from {} by {} : {}".format(self.loader_key, self.logging_metric, self.scores)
            print(msg)
